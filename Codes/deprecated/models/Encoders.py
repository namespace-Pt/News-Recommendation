...
class FIM_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            cdd_news_embedding: hidden vector of each token in news, of size [batch_size, *, signal_length, level, hidden_dim]
            his_news_embedding
            cdd_news_repr: hidden vector of each news, of size [batch_size, *, hidden_dim]
            his_news_repr
        """
        ...
        his_news_batch = x['clicked_title'].long().to(self.device)
        ...
        his_news_repr = self._scaled_dp_attention(self.query_words, his_news_embedding_attn, his_news_embedding_attn).squeeze(
            dim=-2).view(his_news_batch.shape[0], his_news_batch.shape[1], self.hidden_dim)
        return cdd_news_embedding_dilations, his_news_embedding_dilations, cdd_news_reprs, his_news_repr
...
class NRMS_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        cdd_news_batch = x['candidate_title'].long().to(self.device)
        cdd_news_embedding = self.DropOut(self.embedding(cdd_news_batch)).view(
            cdd_news_batch.shape + (self.level, self.hidden_dim))
        cdd_news_embedding, cdd_news_repr = self._multi_head_self_attention(cdd_news_embedding)

        his_news_batch = x['clicked_title'].long().to(self.device)
        his_news_embedding = self.DropOut(self.embedding(his_news_batch)).view(
            his_news_batch.shape + (self.level, self.hidden_dim))
        his_news_embedding, his_news_repr = self._multi_head_self_attention(his_news_embedding)
        return cdd_news_embedding, his_news_embedding, cdd_news_repr, his_news_repr
...
class NPA_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            his_news_repr
        """
        e_u = self.DropOut(self.user_embedding(x['user_index'].long().to(self.device)))
        word_query = self.Tanh(self.wordQueryProject(
            self.RELU(self.wordPrefProject(e_u))))

        cdd_news_batch = x['candidate_title'].long().to(self.device)
        cdd_news_embedding = self.DropOut(self.embedding(
            cdd_news_batch)).view(-1, cdd_news_batch.shape[-1], self.embedding_dim).transpose(-2, -1)
        cdd_news_embedding = self.RELU(self.CNN(
            cdd_news_embedding)).transpose(-2, -1).view(cdd_news_batch.shape + (self.hidden_dim,))
        cdd_news_repr = self._scaled_dp_attention(word_query.view(
            word_query.shape[0], 1, 1, word_query.shape[-1]), cdd_news_embedding, cdd_news_embedding).squeeze(dim=-2)

        his_news_batch = x['clicked_title'].long().to(self.device)
        his_news_embedding = self.DropOut(self.embedding(
            his_news_batch)).view(-1, his_news_batch.shape[-1], self.embedding_dim).transpose(-2, -1)
        his_news_embedding = self.RELU(self.CNN(
            his_news_embedding)).transpose(-2, -1).view(his_news_batch.shape + (self.hidden_dim,))
        his_news_repr = self._scaled_dp_attention(word_query.view(
            word_query.shape[0], 1, 1, word_query.shape[-1]), his_news_embedding, his_news_embedding).squeeze(dim=-2)

        return cdd_news_embedding.unsqueeze(dim=-2), his_news_embedding.unsqueeze(dim=-2), cdd_news_repr, his_news_repr
...
class MHA_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        cdd_news_batch = x['candidate_title'].long().to(self.device)
        cdd_news_embedding = self.DropOut(self.embedding(cdd_news_batch))
        query = self.queryProject(cdd_news_embedding).view(-1,cdd_news_batch.shape[2],self.hidden_dim).transpose(0,1)
        key = cdd_news_embedding.view(-1,cdd_news_batch.shape[2],self.embedding_dim).transpose(0,1)
        value = key
        cdd_news_embedding, _ = self.MHA(query, key, value)
        cdd_news_embedding = cdd_news_embedding.transpose(0,1).view(cdd_news_batch.shape + (self.hidden_dim,))
        multi_head_self_attn_key = torch.tanh(self.keyProject(cdd_news_embedding))
        cdd_news_repr = self._scaled_dp_attention(
            self.query, multi_head_self_attn_key, cdd_news_embedding).squeeze(dim=-2)

        his_news_batch = x['clicked_title'].long().to(self.device)
        his_news_embedding = self.DropOut(self.embedding(his_news_batch))
        query = self.queryProject(his_news_embedding).view(-1,his_news_batch.shape[2],self.hidden_dim).transpose(0,1)
        key = his_news_embedding.view(-1,his_news_batch.shape[2],self.embedding_dim).transpose(0,1)
        value = key
        his_news_embedding, _ = self.MHA(query, key, value)
        his_news_embedding = his_news_embedding.transpose(0,1).view(his_news_batch.shape + (self.hidden_dim,))
        multi_head_self_attn_key = torch.tanh(self.keyProject(his_news_embedding))
        his_news_repr = self._scaled_dp_attention(
            self.query, multi_head_self_attn_key, his_news_embedding).squeeze(dim=-2)

        return cdd_news_embedding.view(news_batch.shape + (self.level, self.hidden_dim)), his_news_embedding.view(news_batch.shape + (self.level, self.hidden_dim)), cdd_news_repr, his_news_repr
...
class CNN_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        cdd_news_embedding = self.DropOut(self.embedding(
            cdd_news_batch)).view(-1, cdd_news_batch.shape[-1], self.embedding_dim).transpose(-2, -1)
        cdd_news_embedding = self.RELU(self.LayerNorm(self.CNN(
            cdd_news_embedding).transpose(-2, -1))).view(cdd_news_batch.shape + (self.hidden_dim,))
        cdd_news_repr = self._scaled_dp_attention(self.query_words, self.Tanh(self.wordQueryProject(cdd_news_embedding)), cdd_news_embedding).squeeze(dim=-2)

        his_news_batch = x['clicked_title'].long().to(self.device)
        his_news_embedding = self.DropOut(self.embedding(
            his_news_batch)).view(-1, his_news_batch.shape[-1], self.embedding_dim).transpose(-2, -1)
        his_news_embedding = self.RELU(self.LayerNorm(self.CNN(
            his_news_embedding).transpose(-2, -1))).view(his_news_batch.shape + (self.hidden_dim,))
        his_news_repr = self._scaled_dp_attention(self.query_words, self.Tanh(self.wordQueryProject(his_news_embedding)), his_news_embedding).squeeze(dim=-2)
...
class RNN_Encoder(nn.Module):
    ...
    def forward(self, news_batch, **kwargs):
        ...
        cdd_news_batch = x['candidate_title'].long().to(self.device)
        cdd_news_embedding = self.embedding(cdd_news_batch).view(-1, cdd_news_batch.shape[-1], self.embedding_dim)
        cdd_news_embedding, output = self.lstm(cdd_news_embedding)
        cdd_news_repr = torch.mean(output[0],dim=0).view(cdd_news_batch.shape[0],cdd_news_batch.shape[1],self.hidden_dim)

        his_news_batch = x['clicked_title'].long().to(self.device)
        his_news_embedding = self.embedding(his_news_batch).view(-1, his_news_batch.shape[-1], self.embedding_dim)
        his_news_embedding, output = self.lstm(his_news_embedding)
        his_news_repr = torch.mean(output[0],dim=0).view(his_news_batch.shape[0],his_news_batch.shape[1],self.hidden_dim)
        return cdd_news_embedding.view(cdd_news_batch.shape + (self.level, self.hidden_dim)), his_news_embedding.view(his_news_batch.shape + (self.level, self.hidden_dim)), cdd_news_repr, his_news_repr
...
class Bert_Encoder(nn.Module):
    ...
    def forward(self, news_batch, **kwargs):
        ...
        """
        cdd_news_batch = x['candidate_title'].long().to(self.device)
        output = self.bert(cdd_news_batch.view(-1, cdd_news_batch.shape[2]), attention_mask=x['candidate_title_pad'].to(self.device).view(-1, news_batch.shape[2]))
        # stack the last level dimensions to form multi-level embedding
        cdd_news_embedding = torch.stack(output['hidden_states'][-self.level:],dim=-2)
        # use the hidden state of the last layer of [CLS] as news representation
        cdd_news_repr = cdd_news_embedding[:,0,-1,:].view(cdd_news_batch.shape[0], cdd_news_batch.shape[1], self.hidden_dim)

        his_news_batch = x['clicked_title'].long().to(self.device)
        output = self.bert(his_news_batch.view(-1, his_news_batch.shape[2]), attention_mask=x['clicked_title_pad'].to(self.device).view(-1, news_batch.shape[2]))
        # stack the last level dimensions to form multi-level embedding
        his_news_embedding = torch.stack(output['hidden_states'][-self.level:],dim=-2)
        # use the hidden state of the last layer of [CLS] as news representation
        his_news_repr = his_news_embedding[:,0,-1,:].view(his_news_batch.shape[0], his_news_batch.shape[1], self.hidden_dim)
...
class Pipeline_Encoder(nn.Module):
    ...
    def forward(self,x):
        ...
        Returns:
            ...
            his_news_repr
        """
        cdd_news_repr = self.news_repr(x['cdd_id'].long().to(self.device))
        his_news_repr = self.news_repr(x['his_id'].long().to(self.device))

        cdd_news_embedding = self.news_embedding(x['cdd_id'].long().to(self.device)).view(x['candidate_title'].shape + (self.level, self.hidden_dim))
        his_news_embedding = self.news_embedding(x['his_id'].long().to(self.device)).view(x['clicked_title'].shape + (self.level, self.hidden_dim))
        return cdd_news_embedding, his_news_embedding, cdd_news_repr, his_news_repr
...
class Encoder_Wrapper(nn.Module):
    ...
    def forward(self,x):
        ...
        news_embedding, news_repr = self.encoder(x)
