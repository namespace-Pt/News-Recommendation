...
#         news_embedding_selection,_ = self.lstm(news_embedding_pretrained.view(-1, news_batch.shape[-1], self.embedding_dim).flip(dims=[-2]),(news_repr_selection.view(1,-1,self.hidden_dim), torch.zeros((1,news_batch.size(0)*news_batch.size(1),self.hidden_dim),device=self.device)))
