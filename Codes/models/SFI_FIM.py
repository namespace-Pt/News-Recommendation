...
# class SFIModel_greedy(nn.Module):
...
#             nn.ReLU(),
#             nn.MaxPool3d(kernel_size=[3,3,3],stride=[2,3,3]),
#             nn.Conv3d(in_channels=32,out_channels=16,kernel_size=[3,3,3],padding=1),
#             nn.ReLU(),
#             nn.MaxPool3d(kernel_size=[3,3,3],stride=[2,3,3])
#         )
        
#         self.learningToRank = nn.Linear(int((int((self.k - 3)/2 + 1) - 3)/2+1),1)
...
#         # attention over concatenated 3 levels embedding, copy tensor required FIXME
#         news_reprs = news_embedding_dilations.transpose(-2,-3).reshape(news_embedding_set.shape[0], self.signal_length, -1)

#         # attention over mean value of 3 levels embedding
#         # news_reprs = torch.mean(news_embedding_dilations, dim=1)


#         news_reprs = self._scaled_dp_attention(self.query_words, news_reprs, news_reprs).squeeze().view(self.batch_size, news_embedding.shape[1], -1)

#         return news_embedding_dilations.view(self.batch_size, news_embedding.shape[1], self.level, self.signal_length, self.filter_num), news_reprs
...
#         news_embedding_dilations, news_reprs = self._HDC(news_embedding)
...
class SFIModel_pipeline(nn.Module):
    def __init__(self,hparams,vocab):
        super().__init__()
        self.name = hparams['name']
        self.metrics = hparams['metrics']
        ...
        # apply 2d-CNN over dilation embeddings
        # self.SeqCNN2D = nn.Sequential(
        #     nn.Conv2d(in_channels=self.filter_num,out_channels=64,kernel_size=[3,3],padding=1),
        #     nn.ReLU(),
        #     nn.MaxPool2d(kernel_size=[3,2],stride=[3,1]),
    ...
    # def _user_encoder(self, his_news_embedding):
    #     """ encode user with 3D-Convolution
    #     Args:
    #         his_news_embedding: tensor of [batch_size, his_size, k, signal_length, filter_num]

    #     Returns:
    #         user_repr: tensor of [batch_size, ?]
    #     """
    #     his_news_embedding = his_news_embedding.transpose(1,2)
    ...
    def _click_predictor(self,cdd_news_reprs, his_news_reprs):
        ...
        scores = self.learningToRank(scores).squeeze()
    ...
    def forward(self,x):
        ...
        score = self._click_predictor(cdd_news_reprs, his_news_reprs).squeeze()
        return score
...
class SFIModel_unified(nn.Module):
    ...
    def _click_predictor(self, fusion_tensors, attn_weights):
        ...
        Returns:
            score: tensor of [batch_size, npratio+1], which is normalized click probabilty
        """
        score_itr = self.learningToRank_itr(fusion_tensors).squeeze()
        score_repr = self.learningToRank_repr(attn_weights).squeeze()
