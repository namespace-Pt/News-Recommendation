...
class NPAModel(nn.Module):
    ...
    def _news_encoder(self,news_batch,word_query):
        ...
        cdd_title_embedding = self.embedding[news_batch].permute(0,2,1)
    ...
    def _user_encoder(self,his_news_batch,news_query,word_query):
        ...
        """
        user_reprs = torch.zeros((self.batch_size,self.filter_num),dtype=torch.float32)
    ...
    def forward(self,x):
        ...
        # squeeze在用负例的时候得改
        if self.npratio > 0:
            cdd_news_reprs = torch.zeros((self.batch_size,self.npratio+1,self.filter_num),dtype=torch.float32)
